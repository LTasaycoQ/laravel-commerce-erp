<x-layout>
    ...
    @forelse ($proveedores as $proveedor)
        <tr>
            ...
            <td>{{ $proveedor->categoria->category_name ?? '---' }}</td>
            <td>{{ $proveedor->destino->destination_name ?? 'Sin asignar' }}</td>
            <td>{{ $proveedor->created_at?->format('d/m/Y') ?? 'Sin registrar' }}</td>
            <td>
                <span class="status-badge {{ ($proveedor->status ?? 'activo') === 'activo' ? 'status-active' : 'status-inactive' }}">
